...
def test_compare_metrics_and_medians():
    ...
    lines = _audit_lines(
        [
            ("t1", "manual", [("task_start", 0), ("product_viewed", 300), ("cart_updated", 600), ("payment_paid", 1000)]),
            ("t2", "agent", [("task_start", 0), ("product_viewed", 100), ("cart_updated", 200), ("payment_paid", 400)]),
        ]
    )
...
def test_compare_median_across_tasks():
    lines = _audit_lines(
        [
            ("t1", "agent", [("task_start", 0), ("product_viewed", 100), ("cart_updated", 100), ("payment_paid", 100)]),
            ("t2", "agent", [("task_start", 0), ("product_viewed", 200), ("cart_updated", 300), ("payment_paid", 200)]),
            ("t3", "agent", [("task_start", 0), ("product_viewed", 900), ("cart_updated", 900), ("payment_paid", 900)]),
        ]
    )
    result = compare_arms(lines)
...
def test_compare_multiple_products_uses_first_viewed():
    lines = _audit_lines(
        [
            ("t1", "agent", [("task_start", 0), ("product_viewed", 50), ("product_viewed", 120), ("cart_updated", 200), ("payment_paid", 500)]),
        ]
    )
    result = compare_arms(lines)
